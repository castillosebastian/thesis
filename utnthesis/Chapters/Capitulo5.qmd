...
Aunque los resultados iniciales con la integración AV-AG son alentadores, es necesario explorar maneras más eficientes de combinar ambos modelos. El flujo de trabajo encadenado que involucra *selección-generación-selección* mostró ser efectivo en los experimentos con *GCM*, pero puede beneficiarse de ajustes adicionales en su implementación.

### Selección dinámica de características

En lugar de aplicar el AG sobre la totalidad de las características de manera uniforme, una opción interesante podría ser implementar un proceso dinámico de selección de características en múltiples etapas. En este enfoque, el AG se aplicaría inicialmente sobre un subconjunto reducido de características, optimizando en función de la estabilidad y relevancia de los subconjuntos seleccionados. Posteriormente, los AVs generarían datos sintéticos solo para las características seleccionadas, reduciendo así el ruido y permitiendo una exploración más eficiente del espacio de búsqueda. En línea con la técnica implementada en este trabajo.
...
Como vimos en los experimentos, ajustar los parámetros del AVC y AG no es un proceso trivial. Particulamente la cantidad de muestras sintéticas en la etapa generativa y el tamaño del cromosoma activo en la etapa de selección resultan parámetros de un impacto crítico en los resultados de la arquitectura. 

Según vimos en los experimentos realizados, el AV genera muestras sintéticas que luego son utilizadas por el AG para la selección de características, pero los modelos no comparten información durante sus respectivas fases de entrenamiento. La **optimización conjunta** permitiría entrenar ambos modelos de forma simultánea, posibilitando una retroalimentación directa y continua entre los procesos de generación de datos sintéticos (por el AV) y la selección de características (por el AG). Es decir, se ajustaría el proceso de generación de datos en función de la capacidad del AG para seleccionar características relevantes, logrando que el AV genere datos específicamente diseñados para maximizar el rendimiento del AG.
...
Una posibilidad es el diseño de un *stack* de AVs y AGs, donde varias instancias de ambos modelos se encadenen de manera secuencial o paralela. En este esquema, el primer AG-AV generaría un conjunto de características y datos sintéticos, que luego alimentaría a un segundo AG con configuraciones más específicas. Este proceso de encadenamiento podría permitir una mayor concentración de la generación y selección, especialmente en conjuntos de datos donde las relaciones entre las características son extremadamente complejas.
